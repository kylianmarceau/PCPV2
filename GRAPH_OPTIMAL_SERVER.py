...
parallel_data = pd.read_csv('ProfileOutputs/PARALLEL_SERVER.txt', skipinitialspace=True)
serial_data = pd.read_csv('ProfileOutputs/SERIAL_SERVER.txt', skipinitialspace=True)

# Filter data for density 0.60
parallel_density_060 = parallel_data[parallel_data['Density'] == 0.60]
serial_density_060 = serial_data[serial_data['Density'] == 0.60]
...
plt.title('Speedup for Density 0.60 vs Grid Size')
...
plt.tight_layout()
plt.savefig('speedup_density_0.60.png', dpi=300)
plt.show()
...
plt.xlabel('Number of Searches (density 0.60)')
plt.ylabel('Speedup (Serial Time / Parallel Time)')
plt.title('Speedup for Density 0.60 vs Number of Searches')
plt.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig('speedup_density_0.60_vs_searches.png', dpi=300)
plt.show()